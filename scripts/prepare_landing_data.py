...
def main():
    ...
    # Load dominant colors (generated by generate_thumbnails.py)
    colors_path = Path("data/images/colors.json")
    colors: dict[str, str] = {}
    if colors_path.exists():
        colors = json.loads(colors_path.read_text())
        print(f"Loaded {len(colors)} dominant colors")
    ...
    for cam in cameras:
        ...
        img_path = _image_path(local_path)
        entry["image"] = img_path
        # Add dominant color if available
        # img_path is like "cameras/Brand/Model/main.jpg", colors key is "Brand/Model"
        color_key = "/".join(img_path.split("/")[1:3])
        if color_key in colors:
            entry["color"] = colors[color_key]
